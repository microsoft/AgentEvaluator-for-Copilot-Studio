...
# (e.g. the demo-scale file in ../../../sample-data/conversationtranscripts.csv).
...
# Exec every remaining code cell in order, up to but excluding the write cell.
# Indices are DERIVED, not hardcoded - the notebook has been refactored before and
# stale hardcoded indices silently broke this test (it ran off the end of the cell
# list and never reached its own assertions).
code_idx = [i for i, c in enumerate(cells_by_index) if c['cell_type'] == 'code']


def _is_write_cell(i):
    src = get_code(i)
    return 'saveAsTable' in src or 'Delta tables written' in src


write_idx = next((i for i in code_idx if i > 7 and _is_write_cell(i)), None)
if write_idx is None:
    print('*** Could not locate the Delta write cell in the notebook')
    sys.exit(1)

build_cells = [i for i in code_idx if i > 7 and i < write_idx]
for idx in build_cells:
    src = get_code(idx)
    try:
        exec(src, ns)
...
# Now run the write step, redirected to the Pandas-only shim which captures
# saveAsTable() calls instead of touching Spark/Delta.
write_src = get_code(write_idx)
...
# Tables the Fabric parser notebook is contracted to produce.
...
# agent_variables is produced by the Dataverse (Direct) Power Query path only;
# report it when present but don't fail the Fabric notebook for its absence.
optional = ['dbo.agent_variables']
...
for tbl in optional:
    if tbl in written:
        pdf = written[tbl]
        print(f'  + {tbl:30}  rows: {len(pdf):>6}   cols: {len(pdf.columns)}  (optional)')
