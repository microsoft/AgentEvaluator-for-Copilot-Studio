...
```bash
python smoketest_files_mode.py                    # the bundled fixture below
python smoketest_files_mode.py /path/to/other.csv # any other transcripts CSV
```

Exit code 0 = pass, non-zero = fail.

Run automatically on every push/PR that touches the parser or this folder — see
[`.github/workflows/parser-smoke-test.yml`](../../../.github/workflows/parser-smoke-test.yml).

## What's in the sample CSV

Twelve synthetic transcripts (all UPNs anonymised to `user-NNN@example.local`),
...
| 5 | Multiple `VariableAssignment` | Variable decomposition |
...
| 9 | **`replyToId` + scalar `intentScore`** | Turn-level intent join; regression for the crash below |
| 10 | **`replyToId` + nested `{"score": n}` `intentScore`** | The other emitted shape |
| 11 | **`replyToId` + `KnowledgeTraceData`** | `knowledge_searched` / `_answered` / `_sources` on turns |
| 12 | **Submitted `feedbackLoop` verdict + comment** | `feedback_verdict`, `feedback_comment` |

> Transcripts 1-8 never set `replyToId`, so the `intent_by_reply` /
> `knowledge_by_reply` join in `build_turns` was dead code under this fixture.
> That is why a crash on scalar `intentScore` went undetected. Transcripts 9-12
> exercise that path; 9 reproduces the crash against the unfixed parser.

## Expected output

```
SMOKE TEST RESULT — row counts per table
  ✓ dbo.agent_sessions              rows:     12   cols: 31
  ✓ dbo.agent_turns                 rows:     24   cols: 17
  ✓ dbo.agent_errors                rows:      1   cols: 6
  ✓ dbo.agent_subagents             rows:      2   cols: 8
  ✓ dbo.agent_catalogue             rows:      9   cols: 3
  ✓ dbo.agent_performance           rows:     12   cols: 57
SMOKE TEST: PASSED ✓
```
